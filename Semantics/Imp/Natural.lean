...
inductive step: Com × State → State → Prop
  ...
  | while₁ₙ {b: Bexp}
    (u: State) (hb: b s = true) (hc: step (c, s) u)
...
theorem deterministic
  (hps: p ⟹ s) (hpt: p ⟹ t): s = t :=
  by induction hps generalizing t with
  | skipₙ => match hpt with | step.skipₙ => rfl
  | assₙ => match hpt with | step.assₙ => rfl
  | catₙ _ _ _ ihc ihd =>
    match hpt with
    | step.catₙ _ hc hd =>
      exact ihd (ihc hc ▸ hd)
