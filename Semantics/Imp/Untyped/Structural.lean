...
inductive Step: Com × State → Com × State → Prop where
  | ass:
    Step (ass x a, s) (skip, s⟪x ≔ a.reduce s⟫)
  ...
  | cond:
    Step (cond b c d, s) (bif b.reduce s then c else d, s)

  | loop:
    Step (loop b c, s) (bif b.reduce s then c;;loop b c else skip, s)

infix:110 " ⇒ " => Step

theorem Step.demo₁:
  (⦃x = 0; while x <= 2 {x = x + 1}⦄, ⟪⟫) ⇒
      (⦃skip; while x <= 2 {x = x + 1}⦄, ⟪⟫⟪"x"≔0⟫) := cat₂ ass
